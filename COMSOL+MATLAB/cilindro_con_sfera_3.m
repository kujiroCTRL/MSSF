function out = model()
%
% cilindro_con_sfera_3.m
%
% Model exported on Apr 16 2025, 16:50 by COMSOL 5.5.0.359.
...
model.label('cilindro_con_sfera_2.mph');
...
model.study('std1').feature('time').set('tlist', study_tspan);

model.sol('sol1').attach('std1');
% weiwd
%model.sol('sol1').feature('v1').set('clist', {study_tspan, '0.004[s]'});
model.sol('sol1').feature('t1').set('tlist', study_tspan);
...
% model.result('pg1').feature('str1').set('arrowscale', 239.59488583232067);
...
mphsave(model, "cilindro_con_sfera_3.mph");
